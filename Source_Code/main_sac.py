...
if __name__ == '__main__':
    ...
    env_name = 'HalfCheetah-v4'  # You can change this to any other environment supported by mujocore or gymnasium
    ...
    agent = Agent(input_dims=env.observation_space.shape, env=env,
            n_actions=env.action_space.shape[0], update_temper=True, temper=0.2, batch_size=256,reward_scale=0.5,alpha = 0.0003, beta = 0.001, chkpt_dir=f'tmp/{env_name}/model')
    ...
    if not load_checkpoint:
        ...
        os.makedirs(f'tmp/{env_name}/rewards', exist_ok=True)
        os.makedirs(f'tmp/{env_name}/analysis', exist_ok=True)

        np.save(f'tmp/{env_name}/rewards/step_rewards.npy', score_history)
        np.save(f'tmp/{env_name}/analysis/temperature.npy', agent.temper_history)
        np.save(f'tmp/{env_name}/analysis/Q1_mean.npy', agent.q1_mean_history)
        np.save(f'tmp/{env_name}/analysis/Q2_mean.npy', agent.q2_mean_history)
        np.save(f'tmp/{env_name}/analysis/critic_loss.npy', agent.critic_loss_history)
        np.save(f'tmp/{env_name}/analysis/actor_loss.npy', agent.actor_loss_history)
        np.save(f'tmp/{env_name}/analysis/value_loss.npy', agent.value_loss_history)
